...
float vitesse_gauche (float distance_ga, float distance_dr , float distance_de , float distance_ar, float dir)
{
	...
	if(dir = 1) vitesse = petit_negatif;
	else vitesse = petit_positif;
...
float vitesse_droite (float distance_ga, float distance_dr , float distance_de , float distance_ar,float dir)
{
	...
	if(dir = 1) vitesse = petit_positif;
	else vitesse = petit_negatif;
...
void position(float *vg, float *vd, float *x, float *y, float *position_angle, double temps)
{
	float distance_parcouru,angle_parcouru,deplacement_x,deplacement_y;
	float pi = 3.14159265;
	...
	float speed, turnrate,d;
	d=0.20;
	speed = (*vg + *vd)/2;
	turnrate = (*vd - *vg)/(2*d);

	distance_parcouru = temps*speed*k;
	angle_parcouru = temps *turnrate*l;
...
void floue(float capteurs[10],float *x,float *y,float *position_angle,float objectif_x,float objectif_y,float *vg,float *vd)
{
      ...
      distance_de= (capteurs[0]+capteurs[1]+capteurs[3])/3;
      distance_ga= (capteurs[2]+capteurs[5])/2;
      distance_dr= (capteurs[4]+capteurs[6])/2;
...
std::cout << "angle" << "   " << vecteur_a << std::endl;
...
int main(int argc, char **argv)
{
  ...
  try
  {
   ...
   /* std::cout << "objectif: distance en devant du robot" << std::endl;
    std::cin >> objectif_x;
    std::cout << "objectif: distance a gauche du robot (distance negative si l'objectif est sur la droite du robot)" << std::endl;
    std::cin >> objectif_y;*/
	...
	position(vg,vd,x,y,position_angle,temps);
