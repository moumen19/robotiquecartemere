...
CvAVIWriter* AVIWriterL = NULL;
CvAVIWriter* AVIWriterR = NULL;
CvAVIWriter* AVIWriterAux = NULL;
...
int main( int argc, char** argv )
{
    ...
    {
        captureL = cvCaptureFromCAM( -1 ); //first camera
    ...
    for(;;)
    {
        ...
        if( !frameL || !frameR )
        {  
            cvReleaseAVIWriter( &AVIWriterAux );

            if( g_write_avi == 1 )
            {
                cvReleaseAVIWriter( &AVIWriterL );
                cvReleaseAVIWriter( &AVIWriterR );
            }
            break;
        }
        ...
        case 'a':
            ...
            if( g_write_avi )
            {
                AVIWriterL = cvCreateAVIWriter( "StereoL.avi", -1,
                                        10, cvSize(frameL->width,frameL->height)  );
                AVIWriterR = cvCreateAVIWriter( "StereoR.avi", -1,
                                        10, cvSize(frameL->width,frameL->height)  );
            }
            else
            {
...
void SaveStereoPair( IplImage** images, const char* rootname )
{
    ...
    for( i = 0; i < 1000; i++ )
    {
        ...
        cvvSaveImage( name, images[0] );

        sprintf( name, "%s%03dR.bmp", rootname, i );
        cvvSaveImage( name, images[1] );
...
void callback( IplImage* image1, IplImage* image2 )
{
    ...
    if( g_write_avi )
    {
        cvWriteToAVI( AVIWriterL, image1 );
        cvWriteToAVI( AVIWriterR, image2 ); 
    }      
#endif
    g_save = 0;
}
...
/*void calc_disparity_matcher(IplImage* left, IplImage* right)
{
   ...
   // iplBlur( stereo_matcher.GetDisparity(), 
    ...
    if( !AVIWriterAux )
    {
        AVIWriterAux = cvCreateAVIWriter( "StereoLD.avi", -1,
    ...
    cvWriteToAVI( AVIWriterAux, merged );
