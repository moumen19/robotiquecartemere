...
int main(int argc, char** argv)
{
  ...
  /*  
    ...
    int color_mode = 0;
    Mat img1 = imread("/home/olivier/projet_vision/L0.jpg", color_mode);
    Mat img2 = imread("/home/olivier/projet_vision/R0.jpg", color_mode);
...
//    Mat img2 = imread(img2_filename, color_mode);
    ...
    Mat disp(img1.rows,img1.cols,CV_8U);
    Mat disp8(img1.rows,img1.cols,CV_8U);
...
int k=0;
//k= img1.at<uchar>(20,20);
//printf("%d\n",k);


	//float k=0;
	Mat max_disp(50,disp8.cols,CV_8U);
	for(int i=0; i< disp8.cols ; i++)
	{
		for(int j=0 ; j< disp8.rows ; j++)
		{
			if((k < disp8.at<uchar>(j,i)) && (disp8.at<uchar>(j,i) < 220)) 
			{
				k = disp8.at<uchar>(j,i);
			}
//printf("%d\t%d\t%d\n",j,i,k);
		}	
		for(int j=0 ; j< 50 ; j++)
		{
			max_disp.at<uchar>(j,i) = k;
		}
		k=0;
	}
	namedWindow("Max Disp", 1);
        imshow("Max disp", max_disp);
	waitKey();
